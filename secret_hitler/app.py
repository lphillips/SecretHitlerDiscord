...
from secret_hitler.game import Game, Player, GameState, VoteResult
...
class GameStateObserver:
    ...
    @staticmethod
    def on_enter_game_start(game, ctx):
        asyncio.create_task(send_game_start_message(game))

    @staticmethod
    def on_enter_nomination(game, ctx):
        asyncio.create_task(send_nomination_message(game))

    @staticmethod
    def on_enter_election(game, nominated_player, ctx):
        asyncio.create_task(send_election_message(game, nominated_player, ctx))

    @staticmethod
    def on_enter_legislative_president(game, ctx):
        asyncio.create_task(send_legislative_presidential_message(game))

    @staticmethod
    def on_enter_legislative_chancellor(game, ctx):
        pass

    @staticmethod
    def on_government_in_chaos(game, ctx):
        asyncio.create_task(send_government_in_chaos_message(game))

    @staticmethod
    def on_enter_veto(game, ctx):
        pass

    @staticmethod
    def on_enter_investigation(game, ctx):
        pass

    @staticmethod
    def on_enter_special_election(game, ctx):
        pass

    @staticmethod
    def on_enter_policy_peek(game, ctx):
        asyncio.create_task(send_policy_peek_message(game))

    @staticmethod
    def on_enter_execution(game):
        pass

    @staticmethod
    def on_enter_game_over(game):
        asyncio.create_task(send_game_over_message(game))
...
async def on_reaction_add(reaction, user):
    ...
    if game.state == GameState.ELECTION:
        ...
        vote = emoji_to_vote(emoji)
        vote_result = game.vote(user.id, vote)

        if vote_result == VoteResult.INVALID:
            await reaction.remove(user)
            return
...
async def rules(ctx):
    await print_rules(ctx.message.channel)


@client.command(name='license')
async def license_command(ctx):
    await print_license(ctx.message.channel)


@client.command(name='help')
async def show_help(ctx):
    await print_help(ctx.message.channel)
...
async def start_game(ctx, mode, players: int):
    ...
    running_games[game_id] = Game(GameStateObserver(), channel.id, game_id, players, ctx.message.author.id)
    running_games[game_id].add_player(ctx.message.author.id, ctx.message.author.display_name,
                                      ctx.message.author.avatar_url)

    embed = discord.Embed(title='Starting Secret Hitler...', description='Waiting for other players')
    embed.add_field(name='Slots', value='1/' + str(players))
    if mode.lower() == 'private':
        embed.add_field(name='Private Game',
                        value='This is a private game. Invite other players by using -invite <playername>. Ensure the '
                              'player is ready to play and on the server',
                        inline=False)
    else:
        embed.add_field(name='Public Game', value='This is a public game. To join it click on the reaction below!',
                        inline=False)
...
async def invite(ctx, member: commands.MemberConverter):
    ...
    embed = discord.Embed(
        title="Secret Hitler",
        description="You have been added to a Secret Hitler game. The round will start as soon "
                    "enough players have joined"
    )
    embed.add_field(
        name="Game channel",
        value="Visit the channel #game_" + str(game.get_id()) + ". The game will be processed in there. You will "
              "receive important information like your role as a Direct Message"
    )
...
async def nominate(ctx, member: commands.MemberConverter):
    ...
    if game.state is not GameState.NOMINATION:
        # TODO error message
...
async def discard(ctx, card):
    ...
    if game.state is not GameState.LEGISLATIVE_PRESIDENT and game.state is not GameState.LEGISLATIVE_CHANCELLOR:
    ...
    if game.state == GameState.LEGISLATIVE_CHANCELLOR:
        embed = discord.Embed(title='The President discarded a policy',
    ...
    if game.state == GameState.NOMINATION:
        game.set_president()
    ...
    if game.fascist_board == 5:
        embed = discord.Embed(
            title='Veto right',
            description="Five fascist policies are enacted. The chancellor can now veto an agenda. "
            "Use -veto to do that",
            color=discord.Color.dark_red()
        )
        await client.get_channel(game.channel_id).send(embed=embed)
    ...
    if game.state == GameState.INVESTIGATION:
        embed = discord.Embed(
            title='Investigation',
            description='The current president can now investigate a players party. Please use -investigate '
                        '<playername> to do that',
            color=discord.Color.dark_red()
        )
        await client.get_channel(game.channel_id).send(embed=embed)
        return

    if game.state == GameState.POLICY_PEEK:
        game.policy_peek()
        ...
        await client.get_user(game.president.player_id).send(embed=embed, file=file)
        embed = discord.Embed(
            title='Policy Peek',
    ...
    if game.state == GameState.EXECUTION:
        embed = discord.Embed(
            title="Execution",
    ...
    if game.state == GameState.SPECIAL_ELECTION:
        embed = discord.Embed(
...
async def president(ctx, user: commands.UserConverter):
    ...
    if game.state is not GameState.SPECIAL_ELECTION:
...
async def investigate(ctx, user: commands.UserConverter):
    ...
    if game.state is not GameState.INVESTIGATION:
        await ctx.send("You can't investigate somebody's role at the moment")
        return
    ...
    embed = discord.Embed(
        title='Investigation',
        description='Player ' + player.display_name + ' is a ' + player.get_party(),
        color=discord.Color.dark_red()
    )
    embed.set_thumbnail(url=player.avatar_url)
...
async def veto(ctx):
    ...
    if game.state is not GameState.LEGISLATIVE_CHANCELLOR:
    ...
    game.state = GameState.VETO

    embed = discord.Embed(
        title="Veto",
        description="The current chancellor requested veto for this agenda. The president needs to accept (-accept) "
                    "or decline (-decline) the veto",
        color=discord.Color.dark_red()
    )
    await client.get_channel(game.channel_id).send(embed=embed)
...
async def accept(ctx):
    ...
    if game.state is not GameState.VETO:
    ...
    embed = discord.Embed(title="Veto accepted", description="All three policies will be discarded",
                          color=discord.Color.dark_red())
    await client.get_channel(game.channel_id).send(embed=embed)
...
async def decline(ctx):
    ...
    embed = discord.Embed(
        title="Veto declined", description="The chancellor has to discard a policy",
        color=discord.Color.dark_red()
    )
    await client.get_channel(game.channel_id).send(embed=embed)
...
async def execute(ctx, user: commands.UserConverter):
    ...
    if game.state is not GameState.EXECUTION:
...
async def restart(ctx):
    ...
    if game.state is not GameState.GAME_OVER:
...
async def start_chancellor_legislative(game: Game):
    ...
    embed = discord.Embed(
        title="Policies",
        description="These are the three new policies. Use -discard <f/l> to discard a policy",
        color=discord.Color.dark_red()
    )
...
async def setup(guild):
    ...
    for emoji in guild.emojis:
        if emoji.name in emojis:
            logger.debug("Found existing emoji: " + emoji.name)
            del emojis[emoji.name]
...
async def send_players_info(game: Game):
    ...
    for i in range(len(game.players)):
        embed.add_field(name='Player ' + str(i + 1), value=game.players[i].display_name)
    await client.get_channel(game.channel_id).send(embed=embed)


async def send_board(game: Game):
    game.print_board()
...
async def send_roles(game: Game):
    hitler = None
    fascists = []
    for player in game.players:
...
async def print_rules(channel):
    embed = discord.Embed(
        title="Rules",
        url="https://cdn.vapid.site/sites/a67e0c72-4902-4365-a899-3386df73c2c4/assets/"
            "Secret_Hitler_Rules-023bc755617986cb2276a3b6920e43e0.pdf",
    ...
    embed.add_field(
        ...
        value="At the beginning of the game, each player is secretly assigned to one of three roles: Liberal, "
              "Fascist, or Hitler. The Liberals have a majority, but they don’t know for sure who anyone is; Fascists "
              "must resort to secrecy and sabotage to accomplish their goals. Hitler plays for the Fascist team, "
              "and the Fascists know Hitler’s identity from the outset, but Hitler doesn’t know the Fascists and must "
              "work to figure them out.The Liberals win by enacting five Liberal Policies or killing Hitler. The "
              "Fascists win by enacting six Fascist Policies, or if Hitler is elected Chancellor after three Fascist "
              "Policies have been enacted. Whenever a Fascist Policy is enacted, the government becomes more "
              "powerful, and the President is granted a single-use power which must be used before the next round can "
              "begin. It doesn’t matter what team the President is on; in fact, even Liberal players might be tempted "
              "to enact a Fascist Policy to gain new powers.",
        inline=False
    )
    embed.add_field(
        name="Object",
        value="Every player has a secret identity as a member of either the Liberal team or the Fascist team. Players "
              "on the Liberal team win if either: Five Liberal Policies are enacted OR Hitler is assassinated. "
              "Players on the Fascist team win if either: Six Fascist Policies are enacted OR Hitler is elected "
...
async def print_license(channel):
    embed = discord.Embed(
        title="Licenses",
    ...
    embed.add_field(
        ...
        value="This project is licensed under Creative Commons BY-NC-SA 4.0. You are free to adapt and share the game "
              "in any form or format under following conditions. You have to credit us if you use this game. You are "
              "not allowed to use the game for commercial use. You have to use the same license as we do (CC BY-NC-SA "
              "4.0). You are not allowed to restrict others from doing anything our license allows. That means you "
              "can't submit the app to an app store without approval",
        inline=False
    )
    embed.add_field(
        name="Credits",
        value="Secret Hitler was created by Mike Boxleiter, Tommy Maranges, Max Temkin, and Mac Schubert (see "
              "secrethitler.com). The graphics used for this project are from secrethitlerfree.de and made by "
              "Flatimalsstudios. The code used for this game is made by Nergon and also licensed under CC BY-NC-SA "
              "4.0",
        inline=False
    )
    embed.add_field(
        name="Source Code",
        value="You can find the source code for this project on github.com/Nergon",
        inline=False
    )
    await channel.send(embed=embed)
...
def emoji_to_vote(emoji: discord.Emoji) -> str:
    if JA in emoji.name:
        return "y"
    elif NEIN in emoji.name:
        return "n"

    raise ValueError("Emoji could not be recognized as a yes or no vote")
...
async def send_game_start_message(game: Game):
    await client.get_channel(game.channel_id).send("All players joined the game. Let's get it started.")
    await send_players_info(game)
    await send_roles(game)
    await send_board(game)


async def send_nomination_message(game: Game):
    embed = discord.Embed(
        title="Starting election",
        description=game.president.display_name + " is president. Please nominate your chancellor candidate!\nUse "
                                                  "`-nominate <username>`",
...
async def send_election_message(game: Game, nominated_player: Player, ctx):
    embed = discord.Embed(
        title='Player ' + nominated_player.display_name + ' was nominated for chancellor',
        description="Please react to this message with Ja or Nein to vote",
        color=discord.Color.dark_red()
    )
    embed.set_thumbnail(url=nominated_player.avatar_url)
    msg = await client.get_channel(game.channel_id).send(embed=embed)
    await msg.add_reaction(discord.utils.get(ctx.guild.emojis, name=JA))
    await msg.add_reaction(discord.utils.get(ctx.guild.emojis, name=NEIN))


async def send_legislative_presidential_message(game: Game):
    embed = discord.Embed(
        title="New Chancellor",
        description=game.chancellor.display_name + " is the new chancellor. Legislative Session starts now",
        color=discord.Color.dark_red()
    )
...
async def send_government_in_chaos_message(game: Game):
    embed = discord.Embed(
...
async def send_policy_peek_message(game: Game):
    file = discord.File(
...
async def send_investigation_message(game: Game):
    embed = discord.Embed(
        title="Investigation",
        description="The current president can now investigate a player's party. "
                    "Please use `-investigate @<username>` to do that",
        color=discord.Color.dark_red()
    )
    await client.get_channel(game.channel_id).send(embed=embed)


async def send_execution_message(game: Game):
    embed = discord.Embed(
        title="Execution",
        description="The current president can execute a player. Please use `-execute @<username>`",
        color=discord.Color.dark_red()
    )
    await client.get_channel(game.channel_id).send(embed=embed)


async def send_game_over_message(game: Game):
    embed = discord.Embed(
        title="Game over!",
        description=game.winner + "s won the game. Use -restart to restart the game",
        color=discord.Color.dark_red()
